...
res <- rQTest(phenotype=pheno, genotype=geno, method="pca", out.type = "D")
res
```
...
pheno <- data$pheno
geno <- data[, 2:dim(data)[2]]
res <- rQTest(phenotype=pheno, genotype=geno, method="pca", out.type = "C")
res
```
...
pheno <- data$pheno
geno <- data[, 2:dim(data)[2]]
res <- rQTest(phenotype=pheno, genotype=geno, method="pls", out.type = "C")
res
```
...
pheno <- data$pheno
geno <- data[, 2:dim(data)[2]]
res <- rQTest(phenotype=pheno, genotype=geno, method="pls", out.type = "D", scale = TRUE)
res
```
...
res <- rQTest(phenotype=pheno, genotype=geno, covariates = covars, method="pca", out.type = "D")
res
```
...
```{r}
data1 <- read.table(system.file("data/test.bin1.dat",package="rqt"), header = TRUE)
data2 <- read.table(system.file("data/test.bin2.dat",package="rqt"), header = TRUE)
data3 <- read.table(system.file("data/test.bin3.dat",package="rqt"), header = TRUE)
...
* Lee, J., Kim, Y.J., Lee, J., T2D-Genes Consortium, Kim, B-J., Lee, S., Park T. (2016) Gene-set association tests for next-generation sequencing data, Bioin-formatics, 32(17).
