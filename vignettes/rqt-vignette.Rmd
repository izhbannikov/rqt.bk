...
usage the \code{rqt} or any other issues.
...
## Data description
### Single dataset ###
In \code{rqt} requires two datasets: phenotype (a n by 1) matrix 
...
### Meta-analysis ###
In meta-analysis, \code{rqt} requires a list of M (M - number 
...
```{r}
library(rqt)
data <- read.table(system.file("data/test.bin1.dat",package="rqt"), 
    header = TRUE)
pheno <- data$pheno
...
```{r}
library(rqt)
data <- read.table(system.file("data/test.cont1.dat",package="rqt"), 
    header = TRUE)
pheno <- data$pheno
geno <- data[, 2:dim(data)[2]]
obj <- rqtClass(phenotype=pheno, genotype=geno)
...
In such cases, the package \code{rqt} 
can accept additional covariates, in form of 
\code{N} \times \code{K} matrix, as provided below:
...
pheno <- data$pheno
geno <- data[, 2:dim(data)[2]]
covars <- read.table(system.file("data/test.cova1.dat",package="rqt"), 
    header=TRUE)
