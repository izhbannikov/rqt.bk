...
rQTest <- function(pheno, geno, perm=0, STT=0.2,weight=FALSE, cumvar.threshold=90, reg.family="binomial", method="pca") {
  ...
  if(perm==0){
    rslt <- try(QTest.one(y=pheno,covadat=NULL,newgeno=geno, STT=STT, cumvar.threshold=cumvar.threshold, reg.family=reg.family),TRUE)
  } else {
    rsltPP <- do.call(rbind, lapply(1:perm, function(k){
                                                          yP <- pheno[sample(1:length(pheno),length(pheno),replace=F)]
                                                          t.res <- QTest.one(y=yP,covadat=NULL, newgeno=geno,STT=STT,weight=weight, cumvar.threshold=cumvar.threshold, reg.family=reg.family)
                                                          if(is.na(t.res)) {
